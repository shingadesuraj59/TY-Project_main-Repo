...
const ExperienceCard = ({ experience, showRecommendationScore = false, onSaveUpdate }) => {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [isSaved, setIsSaved] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  // console.log("saved artcile in expe card : ",)
    ...
    <div className="bg-white rounded-2xl border-2 border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300 h-full flex flex-col min-h-[300px] group hover:border-primary-200">
      ...
      <div className="p-6 pb-4 flex-1">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-bold text-gray-900 text-base truncate">
                {experience.users?.name || 'Anonymous'}
              </p>
          ...
          <button className="p-2 rounded-xl hover:bg-gray-100 transition-all duration-200 opacity-0 group-hover:opacity-100">
